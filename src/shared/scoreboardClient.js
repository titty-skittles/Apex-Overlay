...
function getWsUrlFromQuery(paramName = "ws") {
...
export function createScoreboardClient({
  ...
  onUpdate = () => {},
  onStatus = () => {},
  // Optional: how long without messages before we call it "stale"
  staleMs = 5000,
} = {}) {
  const url = wsUrl || chooseWsUrl({ fallback: import.meta.env.VITE_SCOREBOARD_WS });
  ...
  const rawState = Object.create(null);

    function applyStatePatch(stateObj) {
      const changed = [];
  ...
  function sendRegister() {
    ...
    const msg = { action: "Register", paths };
  ...
  async function connectLoop() {
    while (!stopped) {
      try {
        ...
        ws.addEventListener("error", () => {
          // Errors usually followed by close; status handled there.
          setStatus({ phase: "error" });
        });

        // Wait until closed, then reconnect.
        await new Promise((resolve) => {
          ws.addEventListener("close", () => resolve(), { once: true });
        });
      ...
      // Exponential backoff with cap, plus a tiny jitter
      attempt += 1;
      const base = clamp(250 * Math.pow(2, attempt), 250, 8000);
      const jitter = Math.floor(Math.random() * 250);
      const waitMs = base + jitter;
